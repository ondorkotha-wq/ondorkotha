...
const useTicketDetail = (id?: number | string): UseTicketDetailReturn => {
  ...
  const fetchTicket = async (): Promise<TicketDetail> => {
    ...
    const res = await axiosSecure.get<TicketDetail>(`/admin/tickets/${id}`);
    return res.data;
  };

  const query = useQuery<TicketDetail, AxiosError>({
    queryKey: ["admin-ticket-detail", id, token],
    queryFn: fetchTicket,
